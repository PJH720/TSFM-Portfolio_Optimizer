...
> All 3 analysis notebooks have been fully executed with output artifacts available in `notebooks/`.
> Gradio 2-tab dashboard (`app.py`) is functional: single-asset forecast + multi-asset portfolio optimization.


| Component                                          | Status         |
| -------------------------------------------------- | -------------- |
| Data Pipeline (10-phase, Kaggle + FRED + yfinance) | ✅ Complete     |
| Chronos-2 Inference (with macro covariates)        | ✅ Complete     |
| TimesFM 1.0 Inference (univariate)                 | ✅ Complete     |
| Dual-Model Weighted Ensemble                       | ✅ Complete     |
| Markowitz Mean-Variance Optimizer (cvxpy)          | ✅ Complete     |
| Sector Constraints (GICS ≤30%)                     | ✅ Complete     |
| Gradio Dashboard (2 tabs)                          | ✅ Complete     |
| Walk-Forward Backtest (NB03)                       | ✅ Complete     |
| Final Report                                       | 🟡 In Progress |


---

## 🎯 7-Point Evaluation (A–G Criteria)


| #     | Criterion                  | Status                          | Evidence                                                                                                                                                                                                                                                                                               | Score  |
| ----- | -------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ |
| **A** | Theme & Model Selection    | ✅ **Complete**                  | 5-model comparative analysis (TimesFM 1.0/2.0, Chronos/Chronos-2, Moirai). Dual-model ensemble selected with documented rationale. See [Model Card Analysis](#-후보-모델-카드-분석-candidate-model-card-analysis).                                                                                             | **A**  |
| **B** | Data Preparation           | ✅ **Complete**                  | 10-phase automated pipeline: Kaggle bulk S&P 500 + yfinance fallback + FRED 7 series + Kaggle macro 4 series. Output: `sp500_macro_master.csv` — 100 tickers, 11 covariates, ~123K rows. See `scripts/build_dataset.py`.                                                                               | **A+** |
| **C** | Basic Inference            | ✅ **Complete**                  | NB01: Chronos-2 zero-shot 5-step forecast (AAPL, GPU bfloat16, 21 quantiles). NB03: Walk-forward backtest with ensemble μ. `src/forecast.py`: production dual-model inference with macro covariate injection.                                                                                          | **A**  |
| **D** | Fine-tuning / Quantization | 🟡 **Code Ready, Not Executed** | `scripts/run_experiments.py` Exp 2: NF4 quantization via `BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_quant_type="nf4")` — **fully coded** (524 lines) but `results/` directory absent (not yet run on GPU). NB01 uses bfloat16 inference (documented VRAM savings). No LoRA fine-tuning attempted. | **C+** |
| **E** | Portfolio Integration      | ✅ **Complete**                  | `src/optimize.py`: Markowitz QP via cvxpy — `max μᵀw − γ·wᵀΣw` with budget, long-only, sector (≤30%), name (≤25%) constraints. Multi-solver cascade (OSQP→SCS→ECOS). NB03 backtest: **Sharpe 1.598** vs equal-weight 0.164. `portfolio_diagnostics()` computes E[r], σ, Sharpe, sector breakdown.      | **A**  |
| **F** | Gradio Demo                | ✅ **Complete**                  | `app.py`: 2-tab Blocks layout. Tab 1: single-asset forecast (Plotly, CI band, model comparison). Tab 2: multi-asset optimization (pie chart, progress callbacks, sector toggle). 100-ticker dynamic dropdown, 5 preset examples, error handling.                                                       | **A**  |
| **G** | Reporting                  | 🟡 **In Progress**              | `REPORT.md` draft complete (§1-4 + appendix). Notion template (`Summit - Notion_ReportFormat.md`) ready. Backtest metrics documented. Missing: Gradio screenshots, final Notion upload.                                                                                                                | **B**  |


### 📊 Key Metrics from Notebooks


| Metric                                  | Value                         | Source             |
| --------------------------------------- | ----------------------------- | ------------------ |
| Walk-Forward Ann. Return (AI)           | **57.34%**                    | NB03               |
| Walk-Forward Ann. Return (Equal Weight) | 8.97%                         | NB03               |
| Sharpe Ratio (AI Portfolio)             | **1.598**                     | NB03               |
| Sharpe Ratio (Equal Weight)             | 0.164                         | NB03               |
| Max Drawdown (AI)                       | -20.83%                       | NB03               |
| Ann. Volatility (AI)                    | 32.75%                        | NB03               |
| Rebalancing Frequency                   | Monthly (21 trading days)     | NB03               |
| Backtest Period                         | 2025-03-06 → 2026-03-06       | NB03               |
| Chronos-2 Inference Device              | NVIDIA RTX 2060 SUPER (7.6GB) | NB01               |
| Dataset Size                            | ~123,505 rows × 17 cols       | `build_dataset.py` |
| Ticker Universe                         | 99 S&P 500 tickers            | `build_dataset.py` |
| Macro Covariates                        | 11 (7 FRED + 4 Kaggle)        | `build_dataset.py` |


### 🔍 Gap Analysis — Remaining to 100%


| Gap                                     | Severity     | Effort                 | Resolution                                                                                                                                                                     |
| --------------------------------------- | ------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **(D) Run NF4 quantization experiment** | Medium       | ~30 min (GPU required) | `python scripts/run_experiments.py --exp 2` on CUDA machine. Script is fully written — just needs execution + capture `results/exp2_nf4_vram.csv`.                             |
| **(D) Document VRAM comparison**        | Low          | ~15 min                | Add Exp 2 results (FP32 vs NF4 VRAM, latency delta) to REPORT.md §3 "나만의 한 끗".                                                                                                 |
| **(G) Gradio screenshots**              | Low          | ~10 min                | Run `python app.py`, capture Tab 1 + Tab 2, insert into REPORT.md appendix.                                                                                                    |
| **(G) Notion final upload**             | Low          | ~20 min                | Copy REPORT.md content into Notion template, attach screenshots.                                                                                                               |
| **(D) LoRA fine-tuning**                | Low priority | ~2-4 hrs               | Not attempted. Chronos-2 and TimesFM are zero-shot models — fine-tuning is architecturally optional and not required by the evaluation criteria. Mention as "향후 과제" in report. |


**Bottom line:** You're at **~90% completion**. The critical path to 100% is:

1. Run the NF4 experiment on your GPU machine (~30 min)
2. Take 2 Gradio screenshots (~10 min)
3. Upload to Notion (~20 min)

All code is written. No new implementation needed.
...
- **To-Be (AI 방법론):** `Google TimesFM 1.0` 및 `Amazon Chronos-2`와 같은 대규모 사전 학습된 AI 모델의 Zero-shot 추론을 통한 기대수익률($\mu$) 예측
- **목표:** AI가 예측한 기대수익률과 **표본 공분산(Sample Covariance, 252일 연환산)** 행렬을 결합하여, **Sharpe Ratio를 극대화**하는 최적의 자산 가중치($w^*$)를 산출하고 이를 Gradio 대시보드로 시각화합니다.

> **공분산 추정 방법론:** 본 프로젝트는 252거래일 표본 공분산(Sample Covariance, 연환산)을 사용합니다.

## ✨ 주요 기능 (Key Features)

1. **Zero-Shot AI Forecasting:** Hugging Face의 `TimesFM 1.0` 및 `Chronos-2` 모델을 가중 앙상블(Weighted Ensemble)로 결합하여 S&P 500 주식의 미래 단기 수익률 예측 (Point & Quantile Forecasts). Chronos-2는 거시 공변량(DGS10, VIXCLS 등 11개 시계열)을 네이티브로 활용합니다.
2. **Dynamic Portfolio Optimization:** `cvxpy` 기반의 2차 계획법(Quadratic Programming)을 사용한 최적 가중치 도출. 섹터 제약(GICS 기준 30% 상한) 및 개별 종목 집중 제한(25%) 적용.
3. **Interactive Dashboard:** `Gradio`와 `Plotly`를 연동하여 자산 가격 추이, 예측 밴드(80% Prediction Interval), 최적화된 포트폴리오 비중을 직관적으로 시각화.

## 🛠 기술 스택 (Tech Stack)

- **AI / ML:** `transformers`, `torch`, `timesfm`, `chronos-forecasting`
- **Data Processing:** `pandas`, `numpy`, `yfinance`, `fredapi`, `kaggle`
...
```text
[Data Ingestion — 10-Phase Automated Pipeline]
  👉 Phase 1: Kaggle S&P 500 Bulk CSV + Macro Dataset (automated download)
  👉 Phase 2: Dynamic ticker quality assessment (null-rate scoring → top-N by weight)
  👉 Phase 3: Hybrid price loading (Kaggle fast-path + yfinance fallback + recency supplement)
  👉 Phase 4: GICS Sector/Industry metadata join
  👉 Phase 5-7: FRED (7 series) + Kaggle macro (4 series) merge (FRED primary, Kaggle backup)
  👉 Phase 8-9: Daily trading-date alignment + 3-pass missing value strategy (ffill → bfill → interpolate)
  👉 Phase 10: Save → data/sp500_macro_master.csv (100 tickers, 17 columns, ~123K rows)
      ↓
[AI Forecasting Module — Dual-Model Ensemble]
  👉 Chronos-2 (amazon/chronos-2): 120M params, encoder-only, 8192 context
     - past_covariates: DGS10, VIXCLS, BAMLH0A0HYM2, T10Y2Y, SOFR, UNRATE, CPIAUCSL, ...
     - Output: 9 quantiles (q10–q90) for probabilistic forecasting
  👉 TimesFM 1.0 (google/timesfm-1.0-200m-pytorch): 200M params, decoder-only, 512 context
     - Univariate (Close prices only)
     - Output: point forecast + quantile forecasts
  👉 Weighted Ensemble: user-configurable Chronos/TimesFM blend (Gradio slider)
      ↓
[Risk & Optimization Engine]
  👉 공분산 행렬 (Σ): 252거래일 표본 공분산 (연환산, `src/optimize.py`)
  👉 cvxpy QP: Maximize μᵀw − γ·wᵀΣw
     - 제약조건: Σw=1 (완전 투자), w≥0 (공매도 금지)
     - 섹터 제약: GICS 기준 각 섹터 ≤30%, 개별 종목 ≤25%
     - Multi-solver cascade: OSQP → SCS → ECOS (자동 폴백)
      ↓
[Gradio Web Interface — 2 Tabs]
  👉 Tab 1: Single-Asset Forecast (역사적 가격 + CI 밴드 + 앙상블 예측)
  👉 Tab 2: Portfolio Optimization (파이 차트 + 진단 지표: E[r], σ, Sharpe)
```
...
| 번호  | 파일                                         | 내용                                                             | 핵심 라이브러리            | 실행 상태                |
| --- | ------------------------------------------ | -------------------------------------------------------------- | ------------------- | -------------------- |
| 01  | `01_chronos2_basic_inference.ipynb`        | Chronos-2 Zero-Shot 추론 기초 데모 — GPU bfloat16, 5일 예측, 80% CI 시각화 | `chronos`, `torch`  | ✅ 실행 완료 (7/7 셀 출력)   |
| 02  | `02_data_overview_visualization.ipynb`     | S&P 500 + 거시 데이터 탐색적 분석 (EDA) — 99 tickers, 11 covariates      | `seaborn`, `pandas` | ✅ 실행 완료 (11/11 셀 출력) |
| 03  | `03_portfolio_optimization_backtest.ipynb` | TSFM 앙상블 μ + 표본 공분산 Σ + Sharpe-Max QP + Walk-Forward 백테스트      | `cvxpy`, `plotly`   | ✅ 실행 완료 (15/16 셀 출력) |


### 📁 노트북 출력 아티팩트 (Output Artifacts)

실행된 노트북의 시각화 결과물이 `notebooks/` 디렉토리에 저장되어 있습니다:
...
| `notebooks/03_risk_metrics.png`         | 연환산 수익률, 변동성, Sharpe Ratio 비교    |
| `notebooks/03_interactive_summary.html` | Plotly 인터랙티브 종합 대시보드             |


### 📊 Walk-Forward 백테스트 결과 (NB03)
...
Σ source:      Sample covariance (252-day annualized)
...
사용 데이터: 5년치 일별 OHLCV + FRED 거시 변수 (DGS10, VIXCLS, UNRATE) + Kaggle 거시 확장 (SOFR, M2 등)
```
...
| Day                | 계획                            | 실행 결과                                                                                                            | 상태      |
| ------------------ | ----------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------- |
| **Day 1** (Mar 8)  | TSFM 모델 선정 및 S&P 500 데이터셋 구축  | 10-Phase 자동화 파이프라인 구축 (Kaggle + FRED + yfinance 하이브리드), `sp500_macro_master.csv` 생성 (100 tickers, 11 covariates) | ✅ 완료    |
| **Day 2** (Mar 9)  | HF 모델 로드 및 Zero-shot 추론 실행    | Chronos-2 (공변량 포함) + TimesFM 1.0 듀얼 모델 앙상블 구현, NB01 Chronos-2 기초 데모 완료                                           | ✅ 완료    |
| **Day 3** (Mar 9)  | cvxpy 최적화 엔진 + Gradio 대시보드 구현 | Markowitz QP (섹터 제약 포함), 2-tab Gradio UI, Walk-Forward 백테스트 (NB03)                                               | ✅ 완료    |
| **Day 4** (Mar 10) | 최종 백테스트 분석, 보고서 작성 및 발표 시연    | 보고서 작성 중                                                                                                         | 🟡 진행 중 |
...
이 프로젝트는 **PolyForm Noncommercial License 1.0.0**을 따릅니다 (SPDX: `PolyForm-Noncommercial-1.0.0`). 비상업적 용도 등은 라이선스 조건에 따라 허용되나, 상업적 이용은 별도 허가가 필요할 수 있습니다. 전문은 루트의 `LICENSE` 파일과 [PolyForm Noncommercial 1.0.0](https://polyformproject.org/licenses/noncommercial/1.0.0/)을 참고하세요.
